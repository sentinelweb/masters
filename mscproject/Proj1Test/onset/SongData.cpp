...
float SongData::estimateTempoPartial(int _index,int _start,int _end,int _sumPrevious,int _histSize) {// start end are hop numbers
	int sz = onsetssfHops[_index]->getSize();
	VariableArray* tmp=new VariableArray(100);
	for (int i=1;i<sz;i++) {
		float time = onsetssfHops[_index]->get(i);
		if (time>_start && time<_end) {
			tmp->add(onsetssfHops[_index]->get(i)-onsetssfHops[_index]->get(i-1));
		}
		if (i>_end)  {break;}
	}
	IOIHistogram* ii=new IOIHistogram(tmp->getSize());
	ii->sumPrevious=_sumPrevious;
	for (int i=1;i<tmp->getSize();i++) {
		ii->addElement(tmp->get(i));
	}
	delete tmp;
	ii->histoSize=_histSize;
	float tempoHops=ii->estimateTempo();
	if (tempoHops==1) {return 1;}
	float tempoBPM = this->getTempoStd(bpAnalysis->getBPM( Fs/decimation, tempoHops ),160,80);
	return tempoBPM;
}
...
void SongData::correlateTempo(int index) {
	...
	int beatsSize = (int)((onsetssf->getSize()/(float)this->tempo[index]->tempoHops)+1+20);
	...
	//cout << "sz:" << sz << "beatsSize:" <<beatsSize <<endl;
	for (int onsetIndex=1;onsetIndex<sz;onsetIndex++) {// may want to put bounds checking here and start form 0 - but should do for the mo
		...
		while (tmpSearchPos>0) {
			int beatType = 0;
			while( searchPointer>2 && tmpSearchPos<_onsetssfHops->get(searchPointer-1)  ) {
				searchPointer--;
			}
			...
			if (Stats::abs(diff)<beatSnapMargin) {
				...
				corrScore+=onsetssf->get(tmpSearchPos);
				beatType = 1;
				found++;
		...
		while (tmpSearchPos < maxVal ) {
			...
			tmp_beatTypes[beatCtr]=beatType;
			// put size test here...
			if (beatCtr<0 ||beatCtr>=beatsSize ) {
				cout << "beatctr oversized:"<<beatCtr<< endl;;
				//break;
			}
			if (beatCtr>1) {
				tmp_beatIntervals[beatCtr] = Stats::abs(tmpSearchPos-lastTmpSearchPos);
		...
		if (corrScore>maxCorrScore) {// /(float)beatCtr
		//if (corrValue>maxCorr) {
			maxCorrScore=corrScore;
			maxCorr=corrValue;
			maxBeatSize=beatCtr;
			//cout <<"maxOnsetIndex:"<< onsetIndex <<endl;
			for (int i=0;i<beatsSize;i++) {
	...
	tempo[index]->corrScore=maxCorrScore;
...
TempoData::TempoData(){tempoHops=0;tempoBPM=0;avgTempoHops=0;avgTempoBPM=0;correlation=0;correlationPC=0;score=0;corrScore=0;}
TempoData::~TempoData(){};
